...
func (nhs *notebookHTMLStarter) Run(joinerOp joiner.Operator) error {
	...
	if err != nil {
		return errors.CommonError(err, "can't init *notebookHTML as notebook_html.Operator")
	}

	if err = joinerOp.Join(notebookOp, nhs.interfaceKey); err != nil {
		return errors.CommonError(err, fmt.Sprintf("can't join *notebookHTML as notebook_html.Operator with key '%s'", nhs.interfaceKey))
	}
